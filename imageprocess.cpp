...
void ImageProcess::loadIm(cv::Mat &im)
{
   ...
   Blue_marble = im.clone();
}
    //-------------------------------------------------------------------------------------------------//

int ImageProcess::Homing()
{
    cv::cvtColor(Blue_marble, Blue_marble, CV_BGR2HLS);
    cv::split(Blue_marble,HLS_clone);
    cv::inRange(HLS_clone[0],cv::Scalar(100),cv::Scalar(140),HLS_clone[0]);
    int x_pos_holder = -5;
    for (int x = 0; x < Blue_marble.size().width; x++)
    {
        for (int y = 0; y < Blue_marble.size().height/3; y++)
        {
            if (HLS_clone[0].at<uchar>(x,y) == 255)
            {
               x_pos_holder = x;
    ...
    //cv::cvtColor(HLS_clone,HLS_clone,CV_HLS2BGR);
   // cv::imshow("hls camera", HLS_clone[0]);
    if (x_pos_holder == -5) //base case
        return 2;

    if (x_pos_holder < CAMERA_PIXEL_WIDTH/2 - 2)
        return -1;
    else if(x_pos_holder > CAMERA_PIXEL_WIDTH/2 + 2)
        return 1;
...
int ImageProcess::Get_Biggest_marble(double x, double y, double qx, double qy, double qz, double qw)
{
    double marble_center_point = balls[0][0];
    if (balls.size() == 0 || balls[0][2] < 50)
        return -5;
    if ((marble_center_point - (CAMERA_PIXEL_WIDTH/2)) < 0)
        return -1;
    else if ((marble_center_point - (CAMERA_PIXEL_WIDTH/2)) > 0)
        return 1;
