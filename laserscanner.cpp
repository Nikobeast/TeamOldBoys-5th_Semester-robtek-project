...
void LaserScanner::parseLaserScannerMessage(ConstLaserScanStampedPtr &msg)
{
 ...
 //   float fAngleMax       = float(msg->scan().angle_max());
  ...
  //  std::cout << getClosestDistance(fAngleMin,fAngleMax) << std::endl;
...
float LaserScanner::getCenterDistance()
{
    return(m_vsReadings[m_vsReadings.size()/2].fDistance);
}
